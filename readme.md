...
Перейдите к шагу [Перед началом работы](https://github.com/razsm-git/resetme?tab=readme-ov-file#%D1%81%D0%BE%D0%BE%D0%B1%D1%89%D0%B5%D0%BD%D0%B8%D1%8F-%D0%BE%D0%B1-%D0%BE%D1%88%D0%B8%D0%B1%D0%BA%D0%B0%D1%85%D0%BA%D0%BE%D0%B4%D1%8B-%D0%B2%D0%BE%D0%B7%D0%B2%D1%80%D0%B0%D1%82%D0%B0). Также полезными могут быть разделы [Описание конфигурационных файлов](https://github.com/razsm-git/resetme?tab=readme-ov-file#%D0%BE%D0%BF%D0%B8%D1%81%D0%B0%D0%BD%D0%B8%D0%B5-%D0%BA%D0%BE%D0%BD%D1%84%D0%B8%D0%B3%D1%83%D1%80%D0%B0%D1%86%D0%B8%D0%BE%D0%BD%D0%BD%D1%8B%D1%85-%D1%84%D0%B0%D0%B9%D0%BB%D0%BE%D0%B2) и [Сообщения об ошибках/коды возврата](https://github.com/razsm-git/resetme?tab=readme-ov-file#%D1%81%D0%BE%D0%BE%D0%B1%D1%89%D0%B5%D0%BD%D0%B8%D1%8F-%D0%BE%D0%B1-%D0%BE%D1%88%D0%B8%D0%B1%D0%BA%D0%B0%D1%85%D0%BA%D0%BE%D0%B4%D1%8B-%D0%B2%D0%BE%D0%B7%D0%B2%D1%80%D0%B0%D1%82%D0%B0)
...
**В файле `/etc/nginx/sites-available/nginx_resetme` измените url сайта(директива `server_name`), а также в условии `if ($host !~ ^(example.ru|www.example.ru)$ )`, укажите адрес локальной подсети(директива location /admin - allow) для доступа к /admin**
...
В проекте есть файлы [vars.py](samples/vars_sample.py) и [secret.py](samples/secret_sample.py), которые хранят в себе переменные и логины/пароли.
...
- secret_key_django - ключ django сервера, который перенесён из settings.py (при автоматической установке будет выведен в консоль, при ручной установке его необходимо сгенерировать самостоятельно(50 символов) или скопировать из файла `settings.py` перед импортом проекта resetme)
...
Значение:
Пользователь ввёл разные пароли в поля пароль и подтверждение пароля
...
### Если необходимо подключаться к каталогу ldaps с самоподписным сертификатом, то необходимо:
...
1. Разместите ssl сертификаты в соответствующей директории и настройте `nginx` на корректную работу с ними
2. Измените значения переменных в файле `vars.py`, учитывая ваши индивидуальные требования (справочную информацию см.выше)
- Выберите цвет фона(измените значение переменных). Остальную стилизацию, если она необходима, можно делать средствами CSS, в файле webapp/staticfiles/css/main.css (рядом лежат файлы bootstrap)
  
3. Загрузите собственные файлы логотипов в формате `svg` в папку webapp/resetme/static/images
